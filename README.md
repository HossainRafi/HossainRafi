...
CSS Frameworks & Libraries: `Bootstrap` `Tailwind CSS` `Material Ui` `Styled Components`

Components and UIs:  `Daisy Ui` `Flowbite` `Shadcn UI`  `ANT design` `Chakra UI`

Databases:  `Firebase` `MongoDB` `Mongoose`

Familiar With: `Next JS` `TypeScript` `React Native`

Others Development Technologies: `React Router DOM` `JWT` `Axios` `NPM Packages` `ReChart` `Context API` `DOT ENV`
